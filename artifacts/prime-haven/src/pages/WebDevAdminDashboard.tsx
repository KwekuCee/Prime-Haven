  ...
  const handlePHApproval = async (submissionId: string) => {
    try {
      ...
      const { data, error } = await (supabase as any).rpc('admin_ph_approve_submission', {
        p_submission_id: submissionId, p_points: phPoints, p_dept_label: 'Web Dept',
      });
  ...
  const handleClientAcceptance = async (submissionId: string) => {
    try {
      ...
      const { data, error } = await (supabase as any).rpc('admin_client_accept_submission', {
        p_submission_id: submissionId, p_points: clientPoints, p_dept_label: 'Web Dept',
      });
      if (error) throw error;
  ...
  const handleRevokeSubmission = async (submissionId: string) => {
    try {
      const { data, error } = await (supabase as any).rpc('admin_revoke_submission', {
        p_submission_id: submissionId, p_dept_label: 'Web Dept',
      });
      if (error) throw error;
