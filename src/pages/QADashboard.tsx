...
const QADashboard = () => {
    ...
    const handleClientAcceptance = async (submissionId: string) => {
        try {
            ...
            if (user) {
                await supabase.from('system_logs').insert({ action_type: 'client_acceptance', admin_id: user.id, description: `[QA Global] Client accepted: ${submission.project_name} (+${clientPoints} pts)`, timestamp: new Date().toISOString() });
            }
    ...
    const handleRejectSubmission = async () => {
        ...
        try {
            ...
            if (user) { await supabase.from('system_logs').insert({ action_type: 'submission_rejected', admin_id: user.id, description: `[QA Global] Rejected: ${rejectSubmission.project_name}`, timestamp: new Date().toISOString() }); }
    ...
    const handleClientRejection = async () => {
        ...
        try {
            ...
            if (user) { await supabase.from('system_logs').insert({ action_type: 'client_rejected', admin_id: user.id, description: `[QA Global] Client rejected: ${clientRejectSubmission.project_name}`, timestamp: new Date().toISOString() }); }
    ...
    const handleRequestCorrectionWithNote = async () => {
        ...
        try {
            ...
            if (user) { await supabase.from('system_logs').insert({ action_type: 'correction_requested', admin_id: user.id, description: `[QA Global] Requested correction: ${correctionRequestSubmission.project_name} — Note: ${correctionNote.trim()}`, timestamp: new Date().toISOString() }); }
    ...
    const handleRevokeSubmission = async (submissionId: string) => {
        try {
            ...
            await supabase.from('submissions').update({
                status: 'rejected', points_awarded: 0, rejection_reason: 'Submission revoked by QA', updated_at: new Date().toISOString()
            ...
            if (user) {
                await supabase.from('system_logs').insert({ action_type: 'submission_revoked', admin_id: user.id, description: `[QA Global] Revoked submission: ${submission.project_name} (−${pointsToRevoke} pts from ${submission.designer_name})`, timestamp: new Date().toISOString() });
            }
            toast({ title: 'Submission Revoked', description: `${pointsToRevoke} points deducted.` });
            await loadData();
    ...
    const stats = {
        ...
        pendingQA: submissions.filter(s => !s.ph_approved && s.status !== 'rejected').length,
        critical: submissions.filter(s => s.status === 'correction_requested' || s.status === 'client_rejected').length,
        delivered: submissions.filter(s => s.client_accepted).length,
    };

    const serviceCategories = Array.from(new Set(submissions.map(s => s.service_type))).filter(Boolean);

    if (initialAuthCheck || loading) return <SuperAdminLayout><div className="flex items-center justify-center py-32"><div className="w-10 h-10 rounded-full border-4 border-cyan-500 border-t-transparent animate-spin" /></div></SuperAdminLayout>;
        ...
        <SuperAdminLayout onRefresh={() => loadData()} loading={loading}>
            <div className="p-4 sm:p-6 lg:p-8">
                <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-heading font-bold text-cyan-500 flex items-center gap-2">
                            <ShieldCheck className="w-6 h-6" /> Quality Assurance & PM Operations
                        </h1>
                        <p className="text-xs text-muted-foreground mt-0.5">Global oversight of all deliverables, bottlenecks, and cross-category approvals.</p>
                    </div>
                </div>

                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-cyan-500/50 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Total Lifecycle Tasks</span>
                    ...
                    <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-orange-500/50 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Pending QA Check</span>
                            <Clock className="w-4 h-4 text-orange-500" />
                        </div>
                        <div className="text-3xl font-bold tracking-tight">{stats.pendingQA}</div>
                    </div>
                    <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-red-500/50 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Critical Interventions</span>
                            <AlertTriangle className="w-4 h-4 text-red-500" />
                        </div>
                        <div className="text-3xl font-bold tracking-tight">{stats.critical}</div>
                    </div>
                    <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-emerald-500/50 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Successfully Delivered</span>
                            <CheckCircle className="w-4 h-4 text-emerald-500" />
                        </div>
                        <div className="text-3xl font-bold tracking-tight">{stats.delivered}</div>
                    </div>
                </div>
                ...
                <div className="rounded-xl border border-border/50 bg-card/50 shadow-sm overflow-hidden mb-8">
                    <div className="p-4 sm:p-5 border-b border-border/50 bg-card/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div><h2 className="text-base font-bold flex items-center gap-2"><Search className="w-4 h-4 text-cyan-500" /> Global Review Queue</h2></div>
                        <div className="flex flex-col sm:flex-row gap-2">
                            <div className="relative">
                                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground w-3.5 h-3.5" />
                                <Input placeholder="Search overall queue..." className="pl-8 h-8 text-sm w-full sm:w-40 bg-card/50" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                            </div>
                            <Select value={selectedService} onValueChange={setSelectedService}>
                                <SelectTrigger className="h-8 text-sm w-full sm:w-36 bg-card/50"><SelectValue /></SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="all">Global (All Depts)</SelectItem>
                                    {serviceCategories.map((s: string) => (
                            ...
                            <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="client_rejected">Client Escalation</SelectItem>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-card/40 border-b border-border/30">
                                        ...
                                        <TableHead className="text-right">QA Actions</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {filteredSubmissions.length > 0 ? filteredSubmissions.map((s: any) => (
                                        <TableRow key={s.id} className="border-border/30">
                                            <TableCell className="font-semibold text-sm">
                                                {s.project_name}
                                                {s.parent_submission_id && <Badge variant="outline" className="ml-2 text-[10px] bg-red-500/10 text-red-500">Correction</Badge>}
                                            </TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <Badge variant="outline" className={`text-[10px] uppercase font-bold tracking-wider ${s.client_accepted ? 'border-emerald-500 text-emerald-500' :
                                                    s.status === 'client_rejected' ? 'border-red-500 text-red-500' :
                                                        s.status === 'correction_requested' ? 'border-amber-500 text-amber-500 bg-amber-500/10' :
                                                            s.ph_approved ? 'border-blue-500 text-blue-500' :
                                                                s.status === 'rejected' ? 'border-red-500 text-red-500' :
                                                                    'border-orange-500 text-orange-500'
                                                    }`}>
                                                    {s.client_accepted ? 'Complete' : s.status === 'client_rejected' ? 'Client Rejected' :
                                                        s.status === 'correction_requested' ? 'Correction Requested' : s.ph_approved ? 'Awaiting Client' :
                                                            s.status === 'rejected' ? 'Rejected' : 'Pending QA'}
                                                </Badge>
                                            </TableCell>
                                            <TableCell><span className="font-bold text-cyan-500">{s.points_awarded || 0}</span></TableCell>
                                            <TableCell className="text-xs text-muted-foreground">{format(new Date(s.created_at), 'MMM d, yy')}</TableCell>
                                        ...
                                        <TableRow><TableCell colSpan={7} className="h-32 text-center text-muted-foreground"><ShieldCheck className="w-8 h-8 mx-auto mb-2 opacity-30" />No submissions in current filter.</TableCell></TableRow>
                                    )}
                                </TableBody>
                            </Table>
                        </div>
            ...
            <Dialog open={!!rejectSubmission} onOpenChange={open => { if (!open) setRejectSubmission(null); }}>
                <DialogContent><DialogHeader><DialogTitle>Reject Submission / Log Issue</DialogTitle></DialogHeader>
            ...
            <Dialog open={!!clientRejectSubmission} onOpenChange={open => { if (!open) setClientRejectSubmission(null); }}>
                <DialogContent><DialogHeader><DialogTitle>Client Rejection</DialogTitle><DialogDescription>PH points will be retained for "{clientRejectSubmission?.project_name}"</DialogDescription></DialogHeader>
