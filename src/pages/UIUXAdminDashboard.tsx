...
import {
  MonitorSmartphone, PenTool, LayoutTemplate, Layers, CheckCircle,
  XCircle, Search, Clock, ExternalLink, ImageIcon,
  Edit, ThumbsUp, Figma, Settings, AlertTriangle
} from 'lucide-react';
...
const UIUX_SERVICES = ['app-design', 'uiux-design', 'wireframing', 'prototyping', 'web-design', 'dashboard-design', 'uiux', 'ui-ux'];
...
const UIUXAdminDashboard = () => {
  ...
  const [systemSettings, setSystemSettings] = useState<any>({
    ...
    client_acceptance_points: { value: 65 },
  });
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase.from('submissions').select('*').in('service_type', UIUX_SERVICES).order('created_at', { ascending: false }),
  ...
  const handleRejectSubmission = async () => {
    ...
    try {
      ...
      if (user) await supabase.from('system_logs').insert({ action_type: 'submission_rejected', admin_id: user.id, description: `[UI/UX Dept] Rejected: ${rejectSubmission.project_name}`, timestamp: new Date().toISOString() });
  ...
  const handleClientRejection = async () => {
    ...
    try {
      ...
      if (user) await supabase.from('system_logs').insert({ action_type: 'client_rejected', admin_id: user.id, description: `[UI/UX Dept] Client rejected: ${clientRejectSubmission.project_name}`, timestamp: new Date().toISOString() });
  ...
  const handleRequestCorrectionWithNote = async () => {
    ...
    try {
      ...
      if (user) await supabase.from('system_logs').insert({ action_type: 'correction_requested', admin_id: user.id, description: `[UI/UX Dept] Requested correction: ${correctionRequestSubmission.project_name} — Note: ${correctionNote.trim()}`, timestamp: new Date().toISOString() });
  ...
  const handleRevokeSubmission = async (submissionId: string) => {
    try {
      ...
      if (user) await supabase.from('system_logs').insert({ action_type: 'submission_revoked', admin_id: user.id, description: `[UI/UX Dept] Revoked: ${submission.project_name} (−${pointsToRevoke} pts)`, timestamp: new Date().toISOString() });
  ...
  const getStats = () => ({
    ...
    apps: submissions.filter(s => s.service_type === 'app-design').length,
    web: submissions.filter(s => s.service_type === 'web-design').length,
  ...
  return (
    <SuperAdminLayout onRefresh={() => loadData()} loading={loading}>
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mb-6">
          <h1 className="text-xl sm:text-2xl font-heading font-bold text-fuchsia-500">UX/UI Design Intelligence</h1>
          <p className="text-xs text-muted-foreground mt-0.5">Comprehensive oversight for Wireframing, App Design, and Web Properties</p>
        </div>

        {/* Stats Grid */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-fuchsia-500/50 transition-colors">
            <div className="flex justify-between mb-2">
              <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Total Hand-offs</span>
              <Figma className="w-4 h-4 text-fuchsia-500" />
            </div>
            <div className="text-3xl font-bold tracking-tight">{stats.total}</div>
          </div>
          <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-primary/50 transition-colors">
            <div className="flex justify-between mb-2">
              <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">App Design</span>
              <MonitorSmartphone className="w-4 h-4 text-primary" />
            </div>
            <div className="text-3xl font-bold tracking-tight">{stats.apps}</div>
          </div>
          <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-blue-500/50 transition-colors">
            <div className="flex justify-between mb-2">
              <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Web Interfaces</span>
              <LayoutTemplate className="w-4 h-4 text-blue-500" />
            </div>
            <div className="text-3xl font-bold tracking-tight">{stats.web}</div>
          </div>
          <div className="rounded-xl border border-border/50 bg-card/80 p-5 hover:border-orange-500/50 transition-colors">
            <div className="flex justify-between mb-2">
              <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Pending QA</span>
          ...
          <div className="p-4 sm:p-5 border-b border-border/50 bg-card/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-base font-bold flex items-center gap-2">
                <PenTool className="w-4 h-4 text-fuchsia-500" /> Product Design Pipeline
              </h2>
            </div>
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                ...
                <SelectContent>
                  <SelectItem value="all">Pipeline Access</SelectItem>
                  <SelectItem value="pending">In Quality Assurance</SelectItem>
                  <SelectItem value="ph_approved">Client Hand-off</SelectItem>
                  <SelectItem value="approved">Finalized Delivery</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-card/40 hover:bg-card/40 border-b border-border/30">
                    <TableHead>Project Instance</TableHead>
                    <TableHead>Engineer/Designer</TableHead>
                    <TableHead>Service Module</TableHead>
                    <TableHead>Figma Link</TableHead>
