...
const SERVICE_TO_DISCORD: Record<string, string> = {
  logo: "graphic-design", branding: "graphic-design", print: "graphic-design", flyer: "graphic-design",
  uiux: "app-design", web: "web-dev",
};
...
async function postToDiscord(channelId: string, embed: any): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) { console.error("Discord API error:", res.status, await res.text()); return null; }
...
async function getDesignerDiscordChannels(supabase: any, designerId: string): Promise<string[]> {
  ...
  const skills = (detail.skills || []).map((s: string) => s.toLowerCase());
  const title = (detail.professional_title || "").toLowerCase();
  const channels: Set<string> = new Set();
  if (skills.some((s: string) => ["logo", "branding", "print", "flyer", "graphic"].some(k => s.includes(k))) || title.includes("graphic")) channels.add(DISCORD_CHANNELS["graphic-design"]);
  if (skills.some((s: string) => ["ui", "ux", "app", "figma", "mobile"].some(k => s.includes(k))) || title.includes("ui") || title.includes("ux") || title.includes("app")) channels.add(DISCORD_CHANNELS["app-design"]);
  if (skills.some((s: string) => ["web", "frontend", "react", "html", "css", "javascript"].some(k => s.includes(k))) || title.includes("web") || title.includes("dev")) channels.add(DISCORD_CHANNELS["web-dev"]);
  if (channels.size === 0) return Object.values(DISCORD_CHANNELS);
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!designerId || !uuidRegex.test(designerId)) {
      return new Response(JSON.stringify({ success: false, error: "invalid_request" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }
    if (!isAdmin && designerId !== user.id) {
    ...
    if (notificationType === "new_submission" || notificationType === "start_working") {
      const { data: profile } = await supabase.from("profiles").select("full_name").eq("id", designerId).single();
      ...
      const rawDesignerName = (profile?.full_name || "A designer").slice(0, 100).trim();
      const rawProject = (projectName || "Untitled").slice(0, 200).trim();
      ...
      if (DISCORD_BOT_TOKEN) {
        if (isStartWorking) {
          ...
          const channelId = discordCategory ? DISCORD_CHANNELS[discordCategory] : null;
          const embed = { title: `📋 New Submission`, description: `**${rawDesignerName}** has submitted **"${rawProject}"** for review`, color: 0xfe4c18, fields: [...(serviceType ? [{ name: "Service", value: serviceType, inline: true }] : [])], footer: { text: "Prime Haven • Work Submitted" }, timestamp: new Date().toISOString() };
          if (channelId) {
            await postToDiscord(channelId, embed);
          } else {
            const channels = await getDesignerDiscordChannels(supabase, designerId);
